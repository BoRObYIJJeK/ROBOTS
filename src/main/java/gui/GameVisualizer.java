...
public class GameVisualizer extends JPanel
{
    ...
    private void moveRobot(double velocity, double angularVelocity, double duration)
    {
        ...
        if (w > 0 && h > 0) {
            ...
            // Касание левой или правой границы - разворот по горизонтали
            if (newX <= offset || newX >= w - offset) {
                m_robotDirection = Math.PI - m_robotDirection;
                turned = true;
            }

            // Касание верхней или нижней границы - разворот по вертикали
            if (newY <= offset || newY >= h - offset) {
                m_robotDirection = -m_robotDirection;
                turned = true;
            }

            if (turned) {
                // Нормализуем угол после разворота
                m_robotDirection = asNormalizedRadians(m_robotDirection);
                // Не применяем новые координаты, остаёмся на месте
                return;
            }
        }
        // =================================================
